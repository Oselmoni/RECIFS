...
if (is.list(input[[1]]$mpa)) {

in.mpa = lapply(input[[1]]$mpa, function(x) {

  pox = x[rep(c(T,F), times=length(x)/2)] # get x coord poly
  poy = x[rep(c(F,T), times=length(x)/2)] # get y coord poly

  return(point.in.polygon(reef$LON, reef$LAT, pox, poy, mode.checked=FALSE))
  
  
})

in.mpa = matrix(unlist(in.mpa), ncol=length(in.mpa), byrow=F)

in.mpa = apply(in.mpa, 1, max)!=0
}

if (is.array(input[[1]]$mpa)) {
  datain = input[[1]]$mpa
  if (is.null(dim(input[[1]]$mpa))) {datain = matrix(input[[1]]$mpa, ncol=length(input[[1]]$mpa)) }
  
  in.mpa = apply(datain, 1, function(x) {
...
totICI=c() # total average ICI - mpa
...
for (i in 1:nrow(input[[1]]$digitalreef)) {
  ...
  ICI=apply(gendis,2, function(x) {
    if (sum(is.na(x))/length(x)<0.5) { # check if too many NAs
      return(sum(reef$AREA[x<FstT&in.mpa==T], na.rm=T))
    } else {return(NA)}    } )
