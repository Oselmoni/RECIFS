...
### Id of reefs with adaptive CN
CN = input[[1]]$cn
in.cn = rownames(reef)%in%CN

### Set Fst threshold
FstT = as.numeric(input[[1]]$FstT)

totICI=c() # total average ICI - CN
...
for (i in 1:nrow(input[[1]]$digitalreef)) {
  ...
  ICI=apply(gendis,2, function(x) {
    if (sum(is.na(x))/length(x)<0.5) { # check if too many NAs
      return(sum(reef$AREA[x<FstT&in.cn==T], na.rm=T))
    } else {return(NA)}    } )
