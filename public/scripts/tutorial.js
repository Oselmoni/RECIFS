...
promptInfo = function(envar) {
 ...
 '<br>Generated using data from: '+metaVAR[Evar1].varSource+'<br>Link: <a href="'+metaVAR[Evar1].varLINK+'" target="_blank">'+metaVAR[Evar1].varLINK+'</a>'+
...
AOIPolyDraw.on('drawend', function(evt) {
    ...
    if (mode=='standard') {
      ...
      $.ajax({
        ...
        dataType: 'text',
        url:'/AOItuto',
        data: JSON.stringify({'AOIcoord': AOIcoord, 'RScoord' : RSsource.getFeatures()[0].getGeometry().transform('EPSG:3857', 'EPSG:4326').flatCoordinates }),
    ...
    RSsource.getFeatures()[0].getGeometry().transform('EPSG:4326', 'EPSG:3857').flatCoordinates
...
POIsource.on('addfeature', function(evt) {
    ...
    if (spchars.test(pointName) == true) { // if there are forbidden chars in point name
      ...
      document.getElementById('tutoMOD_9').style.display='block'
    }
  }
...
addCUSTOMdwnL = function() {
  ...
  document.getElementById('tutoMOD_10').style.display = 'block'
  document.getElementById('topBUTdwn').disabled=false
...
// here follow scripts and codes from the tutorial mode
...
var RSpoly =  [ [  [
  ...
  var RSvector = new ol.layer.Vector({
    ...
    RSsource.addFeature(new ol.Feature({
      geometry: new ol.geom.MultiPolygon(RSpoly),
      name: 'RedSEA'
    }))

  map.addLayer(RSvector)

// coordinates default POIs
var coordEI = [ 3888470.550274951, 3443172.4985486628 ]
var coordSE = [ 3831609.2031065295, 3246936.6932109925 ]
var coordPS = [ 4144774.9478568984, 2229850.3268123665 ]
var coordJE = [ 4355081.370772557, 2464578.5156825623 ]


//////////////// Define tutorial transitions /////////////////////

  transitionTUTO1 = function() {
    // hide/unhide dialog boxes
    document.getElementById('tutoMOD_1').style.display = 'none'
    document.getElementById('tutoMOD_2').style.display = 'block'

  }

  transitionTUTO2 = function() {
    // hide dialog box
    document.getElementById('tutoMOD_2').style.display = 'none'
    // add call to dialog box
    document.getElementById('tutoBUT').onclick = function() {transitionTUTO1()}
  }


  var firstTUTO = true // variable to check if first time calling this function
  reefsSource.on('change', function() {

    if (firstTUTO==true) {
    // disable AOI button
    document.getElementById('drawAOIbutton').disabled=true
    document.getElementById('drawAOIbutton').style.backgroundColor='grey'
    // show dialog box
    document.getElementById('tutoMOD_3').style.display = 'block'

    // activate ENV button
    document.getElementById('topBUTenv').disabled=false
 
    // add call to dialog box
    document.getElementById('tutoBUT').onclick = function() {document.getElementById('tutoMOD_3').style.display='block'}
  ...
  transitionTUTO3 = function() {
    // activate dialog box
    document.getElementById('tutoMOD_3').style.display = 'none'
  }


  showENV_tuto = function() {
    // call showenv
    showENV()

    // activate dialog box
    document.getElementById('tutoMOD_4').style.display = 'block'

    // de-activcate buttons
    document.getElementById('topBUTenv').disabled=true

    
  }

  transitionTUTO4 = function() {
    // activate dialog box
    document.getElementById('tutoMOD_4').style.display = 'none'

    // add call to dialog box
    document.getElementById('tutoBUT').onclick = function() {
      document.getElementById('tutoMOD_5').style.display = 'block'
    }
  }


  transitionTUTO5 = function() {
    // de-activate dialog box
    document.getElementById('tutoMOD_5').style.display = 'none'
...
ADMOD_tuto = function() {
  ...
  document.getElementById('ambutton').style.backgroundColor='grey'

  // activate dialog box
  document.getElementById('tutoMOD_6').style.display='block'

}


transitionTUTO6 = function() {
  // de-activate dialog box
  document.getElementById('tutoMOD_6').style.display = 'none'
...
advancedPOST_tuto = function(AOI) {

  // retrieve inputs for advanced mod
  var AM_selvar = document.getElementById('advSELVAR').value 
  var AM_startY = document.getElementById('advSELsy').value 
  var AM_endY = document.getElementById('advSELey').value 
  var AM_monthsCB = document.getElementById('allmonthsCB').checked 
  var AM_function = document.getElementById('advSELfun').value 
  var AM_limCB = document.getElementById('customizeCSLcb').checked 
  var AM_colCB = document.getElementById('customizeCSCcb').checked 

  // check if all inputs are correct, else return (exit function)
  if (AM_selvar!='Sea surface temperature') {
    alert('Select Sea surface temperature as variable.')
    return
  }

  if (AM_startY!='2010') {
    alert('Select 2010 as starting year.')
    return
  }

  if (AM_endY!='2020') {
    alert('Select 2020 as ending year.')
    return
  }

  if (AM_monthsCB==false) {
    alert('Check the box for using all the months.')
    return
  }

  if (AM_limCB==true) {
    alert('Uncheck the box for customizing limits of colorscale.')
    return
  }

  if (AM_colCB==true) {
    alert('Uncheck the box for customizing colors of colorscale.')
    return
  }

  if (AM_function!='Standard deviation') {
    alert('Select "standard deviation" as function to compute.')
    return
  }
  ...
  // if all is done, deactivate button
  document.getElementById('runADVbut').disabled=true
  document.getElementById('runADVbut').style.backgroundColor='grey'


  // add call to dialog box
  document.getElementById('tutoBUT').onclick = function() {
      document.getElementById('tutoMOD_7').style.display = 'block'
    }

}

transitionTUTO7 = function() {
   // de-activate dialog box
   document.getElementById('tutoMOD_7').style.display = 'none'

   // activate point button 
   document.getElementById('topBUTpoi').disabled = false

   // remove environmental panel
   document.getElementById('ENVpan').style.display = 'none'
...
showPOI_tuto = function() {
 // open POI panel
  showPOI()

  // add custom POIs
  POIsource.addFeature(new ol.Feature({  geometry: new ol.geom.Point(coordEI),  name: 'Eilat'}))
POIsource.addFeature(new ol.Feature({  geometry: new ol.geom.Point(coordPS),  name: 'PortSudan'}))
POIsource.addFeature(new ol.Feature({  geometry: new ol.geom.Point(coordJE),  name: 'Jeddah'}))
POIsource.addFeature(new ol.Feature({  geometry: new ol.geom.Point(coordSE),  name: 'SharmElSheik'}))

// open modal
document.getElementById('tutoMOD_8').style.display = 'block'

}

transitionTUTO8 = function() {

    // de-activate dialog box
    document.getElementById('tutoMOD_8').style.display = 'none'
...
transitionTUTO9 = function() {

  // de-activate dialog box
  document.getElementById('tutoMOD_9').style.display = 'none'

 // add call to dialog box
 document.getElementById('tutoBUT').onclick = function() {
 document.getElementById('tutoMOD_9').style.display = 'block'}

 // activate add to download button 
 document.getElementById('addDown').disabled = false
}

transitionTUTO10 = function() {
    // de-activate dialog box
    document.getElementById('tutoMOD_10').style.display = 'none'
...
openDWNwindow_tuto = function() {

  // open dwn window
  openDWNwindow()

  // open modal
  document.getElementById('tutoMOD_11').style.display = 'block'
}

transitionTUTO11 = function() {
  // de-activate dialog box
  document.getElementById('tutoMOD_11').style.display = 'none'
...
launchDWN_tuto = function() {

  // check if checkbox is on

  var dwn_CBPOI = document.getElementById('cbPOI').checked

  // launchDWN
  if (dwn_CBPOI==false) {alert('Check the box to download only the data for the closest reefs to the points of interest.')
return}

  launchDWN()

  // open next modal
  document.getElementById('tutoMOD_12').style.display = 'block'

  // close modeal dwn
  document.getElementById('exportPAN').style.display = 'none'
  document.getElementById('downloadMOD').style.display = 'none'

  // inactivate button dnw
  document.getElementById('topBUTdwn').disabled=true

}

transitionTUTO12 = function() {
  // de-activate dialog box
  document.getElementById('tutoMOD_12').style.display = 'none'

  // activate button sea-currents
  document.getElementById('topBUTsc').disabled=false
...
postSC_tuto = function() {

    postSC()

    // add call to dialog box
    document.getElementById('tutoBUT').onclick = function() {
    document.getElementById('tutoMOD_13').style.display = 'block'
    // de-activate button sc, and hide sc panel
...
transitionTUTO13 = function() {
  // de-activate dialog box
  document.getElementById('tutoMOD_13').style.display = 'none'
  ...
  document.getElementById('topBUTbg').disabled=false
   ...
   // add call to dialog box
  document.getElementById('tutoBUT').onclick = function() {
  document.getElementById('tutoMOD_14').style.display = 'block'}

}


transitionTUTO14 = function() {
  // de-activate dialog box
  document.getElementById('tutoMOD_14').style.display = 'none'

  // in-activate button sea-currents
  document.getElementById('topBUTbg').disabled=true
  document.getElementById('bgpan').style.display='none'

  // activate button dwn
  document.getElementById('topBUTdwn').disabled=false
  document.getElementById('ePDF').disabled=false
  document.getElementById('eCSV').disabled=true

}


exportPDF_tuto = function() {

  exportPDF()

  document.getElementById('tutoMOD_15').style.display = 'block'
}